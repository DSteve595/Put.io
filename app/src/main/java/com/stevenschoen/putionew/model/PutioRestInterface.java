...
public interface PutioRestInterface {
	...
	@FormUrlEncoded
	@POST("/transfers/add")
	void addTransferUrl(@Field("url") String url, Callback<Response> callback);
...
//			getUtils().getRestInterface().zip(PutioUtils.intsToString(ids)); TODO
	...
	public static class PostAddTransferJob extends PutioUploadJob {
		...
		public PostAddTransferJob(PutioUtils utils, String url, Context context, Intent retryIntent) {
			super(new Params(0).requireNetwork(), utils, context, retryIntent);
			this.url = url;
		}
		...
		public void onRun() throws Throwable {
			super.onRun();
			getUtils().getRestInterface().addTransferUrl(url, this);
		}
	}
